...
/* Fix oversized st.metric values */
...
with st.sidebar:
    ...
    st.markdown('<span style="font-size:13px; color:#9E9E9E;">Bird Acoustic Monitoring</span>', unsafe_allow_html=True)
    st.divider()
...
if page == "Nodes":
                ...
                if logs_folder:
                    ...
                    if p_files:
                        ...
                        if pdf is not None and not pdf.empty:
                            ...
                            st.markdown(f"**Battery:** {last['PH_SoCi']:.0f}%")
                            st.markdown(f"**Energy:** {last['PH_WH']:.1f} Wh")
                            st.markdown(f"**Last data:** {pdf['t'].max().strftime('%d %b %Y, %H:%M')}")
                        else:
                            st.markdown("**Status:** No data")
                    else:
                        st.markdown("**Status:** No logs")
                else:
                    st.markdown("**Status:** No logs folder")
            else:
                st.markdown("**Status:** Offline mode")
        except Exception:
    ...
    with st.expander("About KōreroNET", expanded=False):
        features = [
            ...
            ('—', 'On-device edge computing and recording in a single package.'),
            ('—', 'Deployable in remote areas with flexible connectivity.'),
            ('—', 'Supports LoRaWAN, Wi-Fi and LTE networking.'),
            ('—', 'Solar-powered and weather-sealed for harsh environments.'),
        ...
        if not raw_folder:
            ...
            if not files:
                ...
                try:
                    ...
                    # Parse: extract [timestamp] from message, 2 columns only
                    log_rows = []
                    ts_re = re.compile(r"\[(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2})\]\s*(.*)")
                    for line in raw_lines:
                        m = ts_re.match(line.strip())
                        if m:
                            log_rows.append({"Timestamp": m.group(1), "Message": m.group(2)})
                        elif line.strip():
                            log_rows.append({"Timestamp": "", "Message": line.strip()})

                    if log_rows:
                        log_df = pd.DataFrame(log_rows)

                        # Search filter only (no severity — log has no levels)
                        search_text = st.text_input("Search messages", placeholder="Filter…", key=k("log_search"))
                        filtered = log_df
                        if search_text:
