...
#          NEW: Click a heatmap cell to open a mini player (Prev / Play / Next)
#               for the matching chunks (new-format CSVs, ≥ 31 Oct 2025).
...
#   [service_account] ... (standard GDrive service account JSON fields)
# ------------------------------------------------------------
...
try:
    # captures plotly click events inside Streamlit
...
with row_top[1]:
    if st.button("🔄 Refresh from Drive now"):
        ...
        for k in list(st.session_state.keys()):
            if str(k).startswith(("drive_kids::","DRIVE_EPOCH","root_map::","t1_idx::")):
...
def ensure_chunk_cached(chunk_name: str, folder_id: str, subdir: str) -> Optional[Path]:
    ...
    if chunk_name in name_to_id:
        try:
            ...
            return None
    # fuzzy fallback using filename anatomy
...
def _std_legacy(df: pd.DataFrame, kind: str) -> pd.DataFrame:
    ...
    df["Clip"] = np.nan  # no direct clip mapping for legacy root CSVs
...
# ============================================================================
# Snapshot/master logic (Tab 2) — unchanged from earlier except for small refactors
# ============================================================================
SNAP_RE = re.compile(r"^(\d{8})_(\d{6})$", re.IGNORECASE)
...
# =========================
# TAB 1 — Detections (root) with click-to-play
# =========================
with tab1:
    ...
    d = st.date_input("Day", value=options[-1], min_value=options[0], max_value=options[-1], help=help_txt)

    @st.cache_data(show_spinner=False)
    def load_csv(path: str | Path) -> pd.DataFrame:
        return pd.read_csv(str(path))

    def load_and_filter(paths: List[str], kind: str, day_selected: date) -> pd.DataFrame:
        frames = []
        ...
        for p in paths:
            try:
                ...
                # attach chunk folder id if we have it (new-format only)
                std["ChunkDriveFolderId"] = root_map.get(str(p), None)
                frames.append(std)
            except Exception:
                pass
        return pd.concat(frames, ignore_index=True) if frames else pd.DataFrame(columns=["Label","Confidence","ActualTime","Clip","Kind","ChunkDriveFolderId"])

    if st.button("Show results", type="primary", key="tab1_show_btn"):
        with st.spinner("Rendering heatmap…"):
            def render(df: pd.DataFrame, title: str):
                df = df.copy()
                ...
                # Filter rows that contributed to this cell
                cell_rows = df[(df["Label"]==y) & (df["HourLabel"]==x)].copy()
                # playable only if we know Clip + folder id (new-format)
                ...
                if playable.empty:
                    st.info("These detections come from legacy root CSVs (no direct clip paths). Use **Tab 2** to verify by snapshot.")
                    return
            ...
            if src == "BirdNET (bn)":
                render(load_and_filter(bn_by_date.get(d, []), "bn", d), f"BirdNET • {d.isoformat()}")
            elif src == "KōreroNET (kn)":
                render(load_and_filter(kn_by_date.get(d, []), "kn", d), f"KōreroNET • {d.isoformat()}")
            else:
                df_bn = load_and_filter(bn_by_date.get(d, []), "bn", d)
                df_kn = load_and_filter(kn_by_date.get(d, []), "kn", d)
                render(pd.concat([df_bn, df_kn], ignore_index=True), f"Combined (BN+KN) • {d.isoformat()}")
