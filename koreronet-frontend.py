...
- Splash: shows "KōreroNET" and "AUT" for ~2s, then reveals the app.
- Node Select at top (single option: Auckland-Orākei).
- Tab 1: Root CSV heatmaps (Drive or local) with calendar + min confidence.
- Tab 2: Verify using snapshot-date (from Backup/YYYYMMDD_HHMMSS) and master CSVs;
         on-demand audio chunk fetch from Drive (no full directory downloads).
- Tab 3: Power graph from "Power logs" (Drive), stitches latest N logs; dual y-axes: SoC_i (%) and Wh.
...
if "splash_done" not in st.session_state:
    # Minimal warm-up: don’t build everything yet—just show branding
    ...
    # Keep splash visible for ~2 seconds, then proceed
    time.sleep(2.0)
